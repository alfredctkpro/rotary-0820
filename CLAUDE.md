...
**伏筆已閉環**：P11 埋的是「**小龍蝦**」這個名字，P20 安全故事一（「幫我整理 email」→ 信箱清空）就是回收。Hermes 本身不需要再回收，**P11 不要加回「這個名字先記住」那句**（8/13 Alfred 指示捨棄）。

### ⚠️ 兩個安全故事的位置是刻意的（8/15 Alfred 拍板，別移回去）

原本在 Prompt 之後（舊 P15／P16），8/15 移到**三層全部講完之後**（現 P20／P21，工具牆與孫悟空之間）。理由：那兩個 agent **三層都有**，是「沒用好／發過頭」的案例，拿來論證「Prompt 不夠」時序是倒的。移位後一頁各扛一層：

| 頁 | 故事 | 診斷 |
|---|---|---|
| P20 | 「幫我整理 email」→ 信箱清空 | **第二層 Context 沒給夠**（沒人告訴它哪幾封是命根子） |
| P21 | 排第四的預約變第一 | **第三層 Harness 發過頭**（用你剛發的那張門禁卡） |

連帶規則：**P19 工具牆不可再爆雷**（「那個插隊的 agent 出事就出在門禁卡太大」那句已移除，改成「這張卡要發多大，你決定」當鋪墊）；P14 進 Context 的橋改用「入場券拿到了，然後呢？」。

**誠實原則**（務實老手人設的技術實作）：投影片**不寫死任何影片引言**（「這些見解我沒想過」是萬用收束句）；話術用「我只要說一句」，不說「全自動」——P27 目前是例行提醒＋一句話交辦，不是無人排程。

**不砍清單**：P4、P9、P10、P13、P18-P19（塑人＋發工具）、P22、P23、P30-P33、P34／P36。

**需壓回 40 分才動的砍法**（依序）：① P20+P21 兩個安全故事併講 ② P16 桌面有限快速帶過 ③ P11 縮 ④ P25 併入 P24。
